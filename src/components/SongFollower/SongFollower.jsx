...
import { playCorrectSound, playWrongSound } from '../../services/audioService.js'
import { COLOR_MAP } from '../../data/songs.js'
import styles from './SongFollower.module.css'

export default function SongFollower({ song, onSongComplete, onMicStatusChange }) {
  ...
  // Use a counter as key so Framer Motion rerenders the shake correctly each time
  ...
  // Reset the "last detected" gate whenever the target note changes,
  // so back-to-back identical notes can each be detected.
  useEffect(() => {
  ...
  const handleCorrectNote = useCallback(async () => {
    ...
    try {
      ...
      if (nextIndex >= song.notes.length) {
        ...
        setTimeout(() => onSongComplete(starsEarned), 2000)
      } else {
        setFrogLoading(true)
      ...
      if (!songCompleteRef.current) processingRef.current = false
    }
  ...
  const handleWrongNote = useCallback(async () => {
    ...
    try {
      ...
      // Only react every 2nd wrong attempt to avoid being discouraging
      if (newWrong % 2 === 0) {
        playWrongSound()
  ...
  return (
    <div className={styles.container}>
      ...
      {/* Current note indicator — entry/exit animation is on the outer div;
          shake is on an inner div so they don't compete for the same animate target */}
      <AnimatePresence mode="wait">
        ...
        >
          <motion.div
            key={wrongShakeKey}
            animate={wrongShakeKey > 0 ? { x: [-8, 8, -6, 6, 0] } : {}}
            transition={{ duration: 0.4 }}
            className={styles.shakeInner}
          >
      ...
      <PianoKeyboard
        ...
        onKeyClick={(note) => {
          if (!processingRef.current) {
            lastDetectedRef.current = null
            if (note === currentNote.note) handleCorrectNote()
